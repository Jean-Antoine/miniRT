...
/*   Updated: 2024/06/25 16:44:36 by lpaquatt         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
	...
	t_object *object)
{
	...
	if (hit && hit->t < distance &&  hit->object != object)// ombre sur l'objet lui meme est deja geree ..?
		return (TRUE);
	return (FALSE);
}

t_color	ft_lighting(t_light_comp l, t_material material, t_light *light,
	t_scene *scene)
{
	t_color		ambient;
	t_color		eff_color;
...
t_color	ft_get_color_at_point(t_object *object, t_point pt, t_vector eye_v,
	t_scene *scene)
{
	t_light			*light;
	t_light_comp	comp;
	...
	while (light)
	{
		...
		if (ft_is_shadowed(pt, *light, *scene, object) == TRUE)
			out = ft_color_add(out, ft_black());
		else 
			out = ft_color_add(out, ft_lighting(comp, object->material, light, scene));
		light = light->next;
	}
	return (out);
}
